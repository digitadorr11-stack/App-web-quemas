...
export default function DashboardPage() {
  ...
  // Quemas Programadas Activas (Excluye Criminales, Finalizadas y Canceladas)
  const activeBurns = useMemo(
    () => burns.filter((b) => b.burn_type !== 'CRIMINAL' && b.status !== 'FINALIZADA' && b.status !== 'CANCELADA'),
    [burns]
  );
  const finalizedBurnsCount = useMemo(
    () => burns.filter((b) => b.burn_type !== 'CRIMINAL' && b.status === 'FINALIZADA').length,
    [burns]
  );

  const filteredBurns = useMemo(() => {
    return activeBurns.filter((b) => {
      if (statusFilter === 'ALL') return true;
      if (statusFilter === 'EN_REVISION') {
        return b.status === 'PATRULLA_ASIGNADA' || b.status === 'EN_REVISION' || b.status === 'REVISION_COMPLETADA';
      }
      return b.status === statusFilter;
    });
  }, [activeBurns, statusFilter]);
  ...
  return (
    <>
      ...
      <main className="lg:pl-64 flex-1 w-full">
        ...
        {/* Cuadros de Estado e Indicadores de Etapas (Ubicados aquí para filtrar las solicitudes) */}
        <StatsOverview
          burns={activeBurns}
        ...
        {/* Header Bar con Conmutador de Vista (Tarjetas / Tabla) */}
        <div className="flex items-center justify-between mb-4 px-1">
          <div className="text-xs sm:text-sm text-gray-600 font-medium flex items-center gap-2">
            <span>
              Mostrando <strong className="text-slate-900 font-black">{filteredBurns.length}</strong> solicitudes activas en proceso {currentUser.role === 'supervisor_frente' ? `(${currentUser.assigned_front || 'Tu frente'})` : currentUser.role === 'patrulla' ? `(Tu patrulla)` : ''}
            </span>
            {statusFilter !== 'ALL' && (
              <button
                onClick={() => setStatusFilter('ALL')}
                className="text-[11px] bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded-md font-bold transition cursor-pointer"
              >
                ✕ Quitar filtro
              </button>
            )}
          ...
          <div className="flex items-center space-x-1 bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
            <button
              ...
              className={`p-1.5 rounded-lg transition cursor-pointer ${
                viewMode === 'cards'
                  ? 'bg-union-800 text-white shadow font-bold'
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
            ...
            <p className="text-xs text-gray-500 max-w-md mx-auto mt-1">
              Todas las quemas han sido completadas o no hay solicitudes pendientes en esta etapa.
            </p>
            <div className="mt-4 flex items-center justify-center gap-2">
              <Link
                href="/quemas-finalizadas"
                className="px-4 py-2 text-xs font-bold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-xl shadow-sm inline-flex items-center gap-1.5 cursor-pointer"
              >
                <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                <span>Ver Quemas Finalizadas ({finalizedBurnsCount})</span>
              </Link>
              {canCreate && (
                <button
                  onClick={() => setIsNewModalOpen(true)}
                  className="px-4 py-2 text-xs font-bold text-white bg-union-800 hover:bg-union-700 rounded-xl shadow inline-flex items-center gap-1.5 cursor-pointer"
                >
                  <PlusCircle className="w-4 h-4" />
