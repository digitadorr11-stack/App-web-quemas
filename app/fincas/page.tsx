...
export default function FincasPage() {
  ...
  // Estados para Borrado / Limpieza
  const [loteToDelete, setLoteToDelete] = useState<FarmLoteCatalog | null>(null);
  ...
  const handleDeleteLote = async () => {
    if (!supabase || !loteToDelete?.id) return;
    try {
      ...
      if (error) throw error;
      showToast(`Lote ${loteToDelete.finca} - ${loteToDelete.lote} eliminado`);
  ...
  // Borrar Catálogo (Completo o Filtrado)
  const handleClearCatalog = async () => {
    if (!supabase) return;
    if (confirmClearText.trim().toUpperCase() !== 'BORRAR') {
      alert('Debes escribir "BORRAR" para confirmar la eliminación.');
      return;
    }
    ...
    try {
      ...
      if (clearScope === 'FILTERED' && searchQuery.trim()) {
        const ids = filteredLotes.map((l) => l.id).filter(Boolean) as string[];
        if (ids.length === 0) return;

        // Borrar en bloques de 200
        const CHUNK_SIZE = 200;
        ...
        showToast(`Se eliminaron ${ids.length} lotes filtrados correctamente`);
      } else {
        ...
        if (error) throw error;
        showToast('Catálogo de fincas y lotes vaciado completamente');
      }
      ...
      alert(`Error vaciando datos del maestro: ${err.message}`);
    } finally {
      setIsClearing(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              disabled={lotes.length === 0}
              className="bg-rose-50 hover:bg-rose-100 disabled:opacity-50 text-rose-700 border border-rose-200 text-xs font-bold py-2.5 px-3 rounded-2xl transition flex items-center gap-1.5 cursor-pointer shrink-0"
              title="Borrar o vaciar datos del maestro"
            >
              <Trash2 className="w-4 h-4 text-rose-600" />
              <span className="hidden sm:inline">Borrar Maestro</span>
            </button>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={() => setLoteToDelete(item)}
                            className="p-1.5 rounded-lg bg-slate-100 hover:bg-rose-50 text-slate-400 hover:text-rose-600 border border-slate-300 hover:border-rose-200 transition cursor-pointer"
                            title="Eliminar lote del maestro"
                          >
      ...
      {/* MODAL ELIMINAR LOTE INDIVIDUAL */}
      {loteToDelete && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-900">Eliminar Lote</h3>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2 text-xs">
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Finca:</span>
                <span className="font-bold text-slate-900">{loteToDelete.finca}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Lote (U.M.):</span>
                <span className="font-mono font-bold text-slate-900">{loteToDelete.lote}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Área:</span>
                <span className="font-mono font-bold text-emerald-700">{loteToDelete.area_ha.toFixed(2)} Ha</span>
              </div>
              {loteToDelete.variedad && (
                <div className="flex justify-between">
                  <span className="text-slate-500 font-medium">Variedad:</span>
                  <span className="font-mono font-bold text-blue-700">{loteToDelete.variedad}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-200">
              ...
              <button
                ...
                className="bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white font-bold text-xs py-2 px-5 rounded-xl transition shadow-lg shadow-rose-200 flex items-center gap-1.5 cursor-pointer"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>{isDeletingLote ? 'Eliminando...' : 'Eliminar Lote'}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* MODAL VACIAR / BORRAR MAESTRO */}
      {isClearModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-rose-100 border border-rose-200 flex items-center justify-center text-rose-600 shrink-0">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Borrar Datos del Maestro</h3>
                  <p className="text-xs text-rose-600 font-semibold">Acción destructiva permanente</p>
                </div>
              </div>
              <button
                onClick={() => setIsClearModalOpen(false)}
                className="text-slate-400 hover:text-slate-700 transition cursor-pointer text-sm"
              >
                ✕
              </button>
            </div>

            {searchQuery.trim() && filteredLotes.length < lotes.length && (
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700">Selecciona el alcance del borrado:</label>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <button
                    ...
                    className={`p-3 rounded-2xl border text-left transition cursor-pointer ${
                      clearScope === 'FILTERED'
                        ? 'border-rose-500 bg-rose-50/70 text-rose-900 font-bold shadow-sm'
                        : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'
                    }`}
                  >
                    <p className="font-bold">Solo filtrados ({filteredLotes.length})</p>
                    <p className="text-[10px] text-slate-500 truncate mt-0.5">Búsqueda: &ldquo;{searchQuery}&rdquo;</p>
                  </button>
                  ...
                  >
                    <p className="font-bold">Todo el maestro ({lotes.length})</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">Vaciar base de datos completa</p>
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4 text-xs text-rose-900 space-y-2">
              <p className="font-bold">
                {clearScope === 'FILTERED' && searchQuery.trim()
                  ? `Se eliminarán ${filteredLotes.length} lotes correspondientes al filtro actual.`
                  : `Se eliminarán todos los ${lotes.length} lotes registrados en el catálogo.`}
              </p>
              <p className="text-rose-700 text-[11px] leading-relaxed">
                Utiliza esta opción si necesitas reiniciar la zafra o volver a importar un archivo nuevo desde cero.
                Las solicitudes de quema históricas no se verán afectadas.
              </p>
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs font-semibold text-slate-700">
                Para confirmar, escribe <span className="font-bold text-rose-600 font-mono">BORRAR</span> a continuación:
              </label>
              <input
                type="text"
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2.5 text-xs text-slate-900 font-mono uppercase tracking-wider focus:outline-none focus:border-rose-500"
              />
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
              ...
              <button
                type="button"
                disabled={isClearing || confirmClearText.trim().toUpperCase() !== 'BORRAR'}
                onClick={handleClearCatalog}
                className="bg-rose-600 hover:bg-rose-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-xs py-2.5 px-5 rounded-xl transition shadow-lg shadow-rose-200 flex items-center gap-1.5 cursor-pointer"
              >
                <Trash2 className="w-4 h-4" />
                <span>{isClearing ? 'Borrando...' : 'Confirmar y Borrar'}</span>
              </button>
            </div>
          </div>
        </div>
